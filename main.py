...
main_keyboard = ReplyKeyboardMarkup(
    keyboard=[[KeyboardButton(text="/start"), KeyboardButton(text="/meow"),
               KeyboardButton(text="/get_prices"), KeyboardButton(text="/manage_coins")]],
...
@dp.message(Command("get_tracked_prices"))
async def get_tracked_prices(message: types.Message):
    await asyncio.sleep(2)
...
#         return
#
#     await state.update_data(selected_coins=selected_coins, current_coin_index=0)
#     await process_next_coin(message, state)
...
#
#     if not params:
#         if current_state in [UserCoinManagementStates.SETTING_GROWTH_PERCENTAGE, UserCoinManagementStates.SETTING_DECLINE_PERCENTAGE]:
#             price_service = CryptoPriceService()
...
#                                                     rate_percentage_growth=getattr(coin, 'rate_percentage_growth', None),
#                                                     rate_percentage_declines=getattr(coin, 'rate_percentage_declines', None),
#                                                     saved_rate_to_compare=getattr(coin, 'saved_rate_to_compare', None))
#
#             await message.answer("Все выбранные монеты успешно добавлены!", reply_markup=coin_management_keyboard)
#             await state.set_state(UserCoinManagementStates.CHOOSING_ACTION)
#         finally:
#             await session.close()
#
...
#     current_coin_index = user_data.get('current_coin_index', 0)
#     selected_coins = user_data.get('selected_coins', [])
#
#     if not selected_coins or current_coin_index >= len(selected_coins):
#         await message.answer("Все выбранные монеты обработаны.")
...
#
#
# async def process_next_coin(message: types.Message, state: FSMContext):
#     user_data = await state.get_data()
#     current_coin_index = user_data.get('current_coin_index', 0) + 1
#     selected_coins = user_data.get('selected_coins', [])
#
...
#         return
#
#     await state.update_data(current_coin_index=current_coin_index)
#     current_coin = selected_coins[current_coin_index]
#
#     await message.answer(f"Настройка параметров для монеты {current_coin.code}.\n"
#                          "Выберите параметры для установки (через запятую):\n"
#                          "1. Минимальный курс\n"
#                          "2. Максимальный курс\n"
#                          "3. Процент роста\n"
#                          "4. Процент падения\n"
#                          "Или введите 'пропустить' для пропуска настройки параметров этой монеты.")
...
#
#     if not selected_coins or current_coin_index >= len(selected_coins):
#         await message.answer("Произошла ошибка. Начните процесс добавления монет заново.")
#         await state.clear()
#         return
#
#     current_coin = selected_coins[current_coin_index]
