...
class PriceMonitor:
    ...
    async def check_price_conditions(self, user_id: int, coin: Coin, association: UserCoinAssociation, current_price: float):
        conditions_met = []
    ...
    async def process_user(self, user, user_service, current_prices):
        ...
        for coin, association in user_coins:
            if coin.coin_id_for_price_getter in current_prices:
                ...
                await self.check_price_conditions(user.chat_id, coin, association, current_price)
    ...
    async def update_prices_and_notify(self):
        async with db_helper.db_session() as session:
            try:
                ...
                await asyncio.gather(*[self.process_user(user, user_service, current_prices) for user in all_users])
